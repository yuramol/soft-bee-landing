...
export function useSwiperPeekAnimation(swiperInstance: SwiperClass | null, maxWidth: number = BREAKPOINTS.MD) {
  useEffect(() => {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            if (!hasAnimated && window.innerWidth < maxWidth) {
              hasAnimated = true;
